...
/* Self-heal choreography, synced to VO landmarks (294-frame scene, b-track):
   f8-60 chart draws to the dip · f110 warn alert ("the second a model slips?")
   f176 SELF-HEAL panel ("re-picks the stack") · f240-285 recovery tail ("recovers... automatically") */
const ALERT_AT = 110;
const HEAL_AT = 176;
const RECOVER_A = 240;
const RECOVER_B = 285;
...
export const Watch: React.FC = () => {
  ...
  const heal = spring({ frame: frame - HEAL_AT, fps, config: { damping: 14, stiffness: 140 } });
  const swapOp = ramp(frame, HEAL_AT + 12, HEAL_AT + 24);
        ...
        <AppWindow title="Dental Clinic Agent" crumb="Evals" delay={2} w={1400}>
          ...
          <div style={{ display: 'flex', gap: 22 }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {/* SELF-HEAL: Speko re-picks the stack, rate recovers */}
              <div style={{ opacity: heal, transform: `translateY(${interpolate(heal, [0, 1], [16, 0])}px)`, background: hexA(t.mint, 0.09), border: `1px solid ${hexA(t.mint, 0.55)}`, borderRadius: t.radius, padding: '12px 16px', boxShadow: `0 0 0 3px ${hexA(t.mint, 0.06)}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ width: 8, height: 8, borderRadius: 999, background: t.mint, boxShadow: `0 0 8px ${hexA(t.mint, 0.8)}` }} />
                    <span style={{ fontFamily: FONT.mono, fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', color: t.mint }}>Self-heal</span>
                  </span>
                  <span style={{ transform: `scale(${recovered})`, display: 'inline-flex' }}>
                    <Badge kind="pass">Recovered</Badge>
                  </span>
                </div>
                <div style={{ fontFamily: FONT.sans, fontSize: 14, fontWeight: 600, color: t.ink, marginTop: 8 }}>Stack re-picked from live benchmarks</div>
